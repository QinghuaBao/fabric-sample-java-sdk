 ...
 * people, you don't want to use this. If in doubt, ask on the mailing list.
 * </p>
 * <p/>
 * <p>
...
public class ECKey implements Serializable {
	...
	/**
	 * Generates an entirely new keypair. Point compression is used so the resulting
	 * public key will be 33 bytes (32 for the co-ordinate and 1 byte to represent
	 * the y bit).
	 */
	 ...
	 * {@link net.bither.bitherj.crypto.ECKey#ECKey(BigInteger, byte[])}.
	 */
	...
	/**
	 * Creates an ECKey given only the private key bytes. This is the same as using
	 * the BigInteger constructor, but is more convenient if you are importing a key
	 * from elsewhere. The public key will be automatically derived from the private
	 * key.
	 */
	...
	/**
	 * Creates an ECKey given either the private key only, the public key only, or
	 * both. If only the private key is supplied, the public key will be calculated
	 * from it (this is slow). If both are supplied, it's assumed the public key
	 * already correctly matches the public key. If only the public key is supplied,
	 * this ECKey cannot be used for signing.
	 *
	 * @param compressed
	 *            If set to true and pubKey is null, the derived public key will be
	 *            in compressed form.
	 */
	...
	/**
	 * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL
	 * or used by the BitCoin reference implementation in its wallet storage format.
	 */
	public byte[] toASN1() {
		try {
	...
	/**
	 * Gets the raw public key value. This appears in transaction scriptSigs. Note
	 * that this is <b>not</b> the same as the pubKeyHash/address.
	 */
	...
	/**
	 * Clears all the ECKey private key contents from memory. WARNING - this method
	 * irreversibly deletes the private key information. It turns the ECKEy into a
	 * watch only key.
	 */
	 ...
	 * embedded in other data structures in the Bitcoin protocol. The raw components
	 * can be useful for doing further EC maths on them.
	 */
	 ...
	 * {@link net.bither.bitherj.crypto.ECKey.ECDSASignature#toASN1()} instead.
	 ...
	 */
	public ECDSASignature sign0(byte[] input) throws KeyCrypterException {
		return sign(input, null);
	}

	public Signature sign(byte[] input) throws KeyCrypterException {
		ECDSASignature eSignature = sign(input, null);

		return new Signature(eSignature.r, eSignature.s);
	}

	/**
	 * If this global variable is set to true, sign() creates a dummy signature and
	 * verify() always returns true. This is intended to help accelerate unit tests
	 * that do a lot of signing/verifying, which in the debugger can be painfully
	 * slow.
	 */
	@VisibleForTesting
	 ...
	 * Signs the given hash and returns the R and S components as BigIntegers. In
	 * the Bitcoin protocol, they are usually encoded using DER format, so you want
	 * {@link net.bither.bitherj.crypto.ECKey.ECDSASignature#encodeToDER()} instead.
	 * However sometimes the independent components can be useful, for instance, if
	 * you're doing to do further EC maths on them.
	 *
	 * @param aesKey
	 *            The AES key to use for decryption of the private key. If null then
	 *            no decryption is required.
	 * @throws net.bither.bitherj.crypto.KeyCrypterException
	 *             if this ECKey doesn't have a private part.
	 */
	public ECDSASignature sign(byte[] input, KeyParameter aesKey) throws KeyCrypterException {
		if (FAKE_SIGNATURES)
	...
	public static boolean verify(byte[] data, ECDSASignature signature, byte[] pub) {
		...
		try {
			...
			// are inherently invalid/attack sigs so we just fail them here rather than
			// crash the thread.
			e.printStackTrace();
			return false;
		}
	...
	private static BigInteger extractPrivateKeyFromASN1(byte[] asn1privkey) {
		// To understand this code, see the definition of the ASN.1 format for EC
		// private keys in the OpenSSL source
		// code in ec_asn1.c:
		//
		// ASN1_SEQUENCE(EC_PRIVATEKEY) = {
		// ASN1_SIMPLE(EC_PRIVATEKEY, version, LONG),
	 ...
	 */
	public String signMessage(byte[] message) throws KeyCrypterException {
		byte[] sigData = signHash(message);
		return new String(Base64.encode(sigData), Charset.forName("UTF-8"));
	}

	public byte[] signHash(byte[] hash) throws KeyCrypterException {
		ECDSASignature sig = sign(hash, null);
	 ...
	 * encoded in base64, returns an ECKey containing the public key that was used
	 * to sign it. This can then be compared to the expected public key to determine
	 * if the signature was correct. These sorts of signatures are compatible with
	 * the Bitcoin-Qt/bitcoind format generated by signmessage/verifymessage RPCs
	 * and GUI menu options. They are intended for humans to verify their
	 * communications with each other, hence the base64 format and the fact that the
	 * input is text.
	 *
	 * @param message
	 ...
	 * {@link net.bither.bitherj.crypto.ECKey#signedMessageToKey(String, String)}.
	 * If the key derived from the signature is not the same as this one, throws a
	 * SignatureException.
	 */
	...
	@Override
	public int hashCode() {
		// Public keys are random already so we can just use a part of them as the
		// hashcode. Read from the start to
		// avoid picking up the type code (compressed vs uncompressed) which is tacked
		// on the end.
	...
	public class Signature {
		...
		public byte[] Serialize() {
			byte[] rb = canonicalizeInt(r);
			byte[] sb = canonicalizeInt(s);

			int length = 6 + rb.length + sb.length;
			...
			System.arraycopy(rb, 0, b, 4, rb.length);
			int offset = rb.length + 4;
			b[offset] = 0x02;
			b[offset + 1] = (byte) (sb.length);
			System.arraycopy(sb, 0, b, offset + 2, sb.length);
			return b;
		}

		public byte[] canonicalizeInt(BigInteger integer) {
			byte[] b = integer.toByteArray();
			if (b.length == 0) {
				b = new byte[] { 0x00 };
			}
			if ((b[0] & 0x80) != 0) {
